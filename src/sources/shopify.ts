...
import type { CanonicalProduct, ReadOnlySourceAdapter, SourceConfig } from "./types.ts";
...
const PRODUCTS_QUERY = `
  query TrackedProducts($first: Int!, $after: String) {
...
function normalize(config: ShopifyConfig, product: ShopifyProduct): CanonicalProduct {
...
export const shopifySourceAdapter: ReadOnlySourceAdapter<ShopifyConfig> = {
  ...
  async *fetchProducts(config, context) {
    ...
    do {
      ...
      queryUrl.searchParams.set("variables", JSON.stringify({ first: 100, after }));
