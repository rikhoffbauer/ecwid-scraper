...
import type { ProductOfferingInput, ReadOnlySourceAdapter, SourceConfig, SourceKind } from "./types.ts";

export interface MarktplaatsConfig extends SourceConfig {
  kind: "marktplaats";
}
...
function normalize(config: MarktplaatsConfig, record: Record<string, unknown>, pageUrl: string): ProductOfferingInput | null {
...
export function parseJsonLdProducts(config: MarktplaatsConfig, html: string, pageUrl = config.url): ProductOfferingInput[] {
...
export function createMarktplaatsAdapter(): ReadOnlySourceAdapter<MarktplaatsConfig> {
  async function* fetchPage(config: MarktplaatsConfig, url: string, context: Parameters<NonNullable<ReadOnlySourceAdapter<MarktplaatsConfig>["searchProducts"]>>[2]) {
    const response = await context.http.fetch(url, {
      headers: { accept: "text/html,application/xhtml+xml" }
    });
    if (!response.ok) throw new Error(`marktplaats source ${config.id} failed: ${response.status} ${response.statusText}`);
    const chunk = parseJsonLdProducts(config, await response.text(), response.url || url);
    if (chunk.length > 0) yield chunk;
  }
  return {
    kind: "marktplaats",
    async *fetchProducts(config, context) {
      yield* fetchPage(config, config.url, context);
    },
    async *searchProducts(config, input, context) {
      const template = config.settings?.searchUrlTemplate;
      if (typeof template !== "string" || !template.includes("{query}")) {
        throw new Error(`Source ${config.id} does not support direct search; configure settings.searchUrlTemplate with {query}`);
      }
      yield* fetchPage(config, template.replaceAll("{query}", encodeURIComponent(input.query)), context);
    }
  };
}
