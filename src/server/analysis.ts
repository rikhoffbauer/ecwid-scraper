import type { AnalysisSnapshot, DealCandidate, LoadedProduct, PriceIndexRow, ProductCluster } from "../shared/types.ts";
...
export function clusterProducts(products: LoadedProduct[], threshold: number): ProductCluster[] {
  ...
  const canonicalClusters = new Map<number, ProductCluster>();
  const clustersByToken = new Map<string, Set<ProductCluster>>();
  const clustersBySku = new Map<string, Set<ProductCluster>>();
  const clustersByCategory = new Map<string, Set<ProductCluster>>();

  const addToIndex = (index: Map<string, Set<ProductCluster>>, key: string | undefined, cluster: ProductCluster) => {
    if (!key) return;
    const matches = index.get(key) ?? new Set<ProductCluster>();
    matches.add(cluster);
    index.set(key, matches);
  };

  const indexCluster = (cluster: ProductCluster, item: LoadedProduct) => {
    for (const token of signature(item)) addToIndex(clustersByToken, token, cluster);
    addToIndex(clustersBySku, productSku(item), cluster);
    addToIndex(clustersByCategory, categoryText(item), cluster);
  };

  for (const item of products) {
    if (item.canonicalProductId && item.canonicalMatchReviewState === "accepted") {
      const existing = canonicalClusters.get(item.canonicalProductId);
      if (existing) {
        existing.products.push(item);
        existing.confidence = Math.min(existing.confidence, item.canonicalMatchConfidence ?? 1);
      } else {
        const cluster = {
          id: `canonical-${item.canonicalProductId}`,
          label: productName(item.product),
          products: [item],
          tokenSignature: "",
          confidence: item.canonicalMatchConfidence ?? 1,
          matchType: "canonical" as const,
          storeIds: [item.storeId]
        };
        canonicalClusters.set(item.canonicalProductId, cluster);
        clusters.push(cluster);
      }
      continue;
    }
    let best: { cluster: ProductCluster; score: number } | null = null;
    const candidates = new Set<ProductCluster>();
    const itemSignature = signature(item);
    const tokenBuckets = [...itemSignature].map((token) => clustersByToken.get(token) ?? new Set<ProductCluster>()).sort((a, b) => a.size - b.size);
    if (threshold >= .5 && tokenBuckets.length) {
      const [smallest, secondSmallest] = tokenBuckets;
      for (const cluster of smallest ?? []) {
        if (!secondSmallest || secondSmallest.has(cluster)) candidates.add(cluster);
      }
    } else {
      for (const bucket of tokenBuckets) for (const cluster of bucket) candidates.add(cluster);
    }
    for (const cluster of clustersBySku.get(productSku(item) ?? "") ?? []) candidates.add(cluster);
    if (threshold <= .1) for (const cluster of clustersByCategory.get(categoryText(item)) ?? []) candidates.add(cluster);
    if (threshold <= 0) for (const cluster of clusters) candidates.add(cluster);
    ...
    if (best && best.score >= threshold) {
      ...
      best.cluster.tokenSignature = [...new Set([...tokenize(best.cluster.tokenSignature), ...itemSignature])].sort().join(" ");
      indexCluster(best.cluster, item);
    } else {
      const cluster = { id: `heuristic-${clusters.length + 1}`, label: productName(item.product), products: [item], tokenSignature: [...itemSignature].sort().join(" "), confidence: 1, matchType: "heuristic" as const, storeIds: [item.storeId] };
      clusters.push(cluster);
      indexCluster(cluster, item);
    }
  }
