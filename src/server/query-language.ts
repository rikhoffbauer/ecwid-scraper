...
export function compileProductQueryToSQL(source: string): CompiledSQLQuery {
  const clean = source.trim();
  if (!clean) return { sql: "1=1", params: [], isSimpleText: true };
  ...
  if (isSimpleText) {
    ...
    return {
      sql: `(
        offering_summary LIKE ? OR
        json_extract(offering_summary, '$.summary.name') LIKE ? OR
        json_extract(offering_summary, '$.summary.sku') LIKE ? OR
        json_extract(offering_summary, '$.name') LIKE ? OR
        json_extract(offering_summary, '$.sku') LIKE ?
      )`,
  ...
  try {
    ...
    function resolveIdent(path: string): string {
      return `COALESCE(json_extract(offering_summary, '$.summary.${path}'), json_extract(offering_summary, '$.${path}'))`;
    }

    function visit(node: Expr): string {
      switch (node.kind) {
        case "literal":
          params.push(node.value);
          return "?";
        case "regex":
          throw new Error("Regular expressions are not supported in database searches.");
        case "ident":
          return resolveIdent(node.path);
        case "unary":
          if (node.op === "!") return `(NOT ${visit(node.expr)})`;
        ...
        case "binary":
          ...
          if (node.op === "^=") return `(${left} LIKE (${right} || '%'))`;
          if (node.op === "$=") return `(${left} LIKE ('%' || ${right}))`;
          if (node.op === "*=" || node.op === "~=" || node.op === "|=") return `(${left} LIKE ('%' || ${right} || '%'))`;
          
          return `(${left} ${node.op} ${right})`;
      }
    }

    const sql = visit(expr);
    return { sql, params, isSimpleText: false };
  } catch (error) {
    return { sql: "1=0", params: [], isSimpleText: false, error: (error as Error).message };
  }
}
